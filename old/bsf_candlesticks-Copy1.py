...
'''
def add_candle_patterns_old(df, o="Open", h="High", l="Low", c="Close", v="Volume" ,pattern_window=5,
...
def add_candle_patterns(
    df, o="Open", h="High", l="Low", c="Close", v="Volume", pattern_window=5,
    doji_thresh=0.1, hammer_thresh=0.25, marubozu_thresh=0.05,
    long_body=0.6, small_body=0.25, shadow_ratio=2.0, near_edge=0.25, rng_thresh=1e-4
    ):
    """
    ...
    #roll_first = df[o].rolling(pattern_window, min_periods=pattern_window).apply("first", raw=True)
    #roll_last = df[c].rolling(pattern_window, min_periods=pattern_window).apply("last", raw=True)
    roll_first = df[o].rolling(pattern_window, min_periods=pattern_window).apply(lambda x: x[0], raw=True)
    roll_last  = df[c].rolling(pattern_window, min_periods=pattern_window).apply(lambda x: x[-1], raw=True)

    #roll_first = df[o].rolling(pattern_window, min_periods=pattern_window).agg("first")
    #roll_last  = df[c].rolling(pattern_window, min_periods=pattern_window).agg("last")

    roll_max = df[h].rolling(pattern_window, min_periods=pattern_window).max()
    roll_min = df[l].rolling(pattern_window, min_periods=pattern_window).min()
    ...
    av = df[v].rolling(20, min_periods=1).mean()
    hv = (df[v] > 1.5 * av).to_numpy()
    lv = (df[v] < 0.7 * av).to_numpy()
    ...
    # Trend detection
    downtrend = (pd.Series(C).shift(1).rolling(pattern_window)
                 .apply(lambda x: x.iloc[-1] < x.iloc[0], raw=False)).to_numpy(dtype=bool)
    uptrend = (pd.Series(C).shift(1).rolling(pattern_window)
               .apply(lambda x: x.iloc[-1] > x.iloc[0], raw=False)).to_numpy(dtype=bool)
    ...
    new_cols.update({
        ...
        "NearHigh": H >= pd.Series(H).rolling(pattern_window).max().to_numpy() * (1 - near_edge),
        "NearLow": L <= pd.Series(L).rolling(pattern_window).min().to_numpy() * (1 + near_edge),
        ...
        "ClimacticCandle": rng > pd.Series(rng).rolling(pattern_window).mean().to_numpy() * 2,
    })
...
def add_signal_strength_old(df, directional_groups=None):
    """
...
'''
def finalize_signals_old(df, pattern_window=5, momentum_factor=0.5,
...
def finalize_signals(df, pattern_window=5, momentum_factor=0.5,
    ...
    # --- Pattern scoring ---
    if bullish_patterns and bearish_patterns:
        bull = df[bullish_patterns].rolling(pattern_window, min_periods=1).sum().sum(axis=1)
        bear = df[bearish_patterns].rolling(pattern_window, min_periods=1).sum().sum(axis=1)
    else:
        bull = pd.Series(0, index=df.index)
        bear = pd.Series(0, index=df.index)
