...
# Add Trends
# -------------------------------
def add_trend_filters(sdf, timeframe="Daily", user: int = 1):
    """
...
# -------------------------------
def add_signal_strength(sdf, directional_groups=None):
    """
    Adds SignalStrength counts and percentages for each row in Spark.
    """

    # 1️⃣ All valid signal columns
    valid_cols = [c for c in sdf.columns if c.startswith("Valid")]
    if not valid_cols:
        sdf = sdf.withColumn("SignalStrength", F.lit(0)) \
    ...
    # Convert all valid signal columns to integer (0/1)
    for c in valid_cols:
        sdf = sdf.withColumn(c, F.col(c).cast("integer"))

    # Count of all signals
    sdf = sdf.withColumn("SignalStrength", sum(F.col(c) for c in valid_cols))
    ...
    sdf = sdf.withColumn("SignalStrengthNonZero", F.when(F.col("SignalStrength") == 0, 1).otherwise(F.col("SignalStrength")))

    sdf = sdf.withColumn("BullishPctRaw", sum(F.col(c) for c in bullish_cols) / F.col("SignalStrengthNonZero"))
    sdf = sdf.withColumn("BearishPctRaw", sum(F.col(c) for c in bearish_cols) / F.col("SignalStrengthNonZero"))
    ...
    directional_sum_expr = sum(F.col(c) for c in directional_cols)
    sdf = sdf.withColumn("DirectionalSumNonZero", F.when(directional_sum_expr == 0, 1).otherwise(directional_sum_expr))

    sdf = sdf.withColumn("BullishPctDirectional", sum(F.col(c) for c in bullish_dir_cols) / F.col("DirectionalSumNonZero"))
    sdf = sdf.withColumn("BearishPctDirectional", sum(F.col(c) for c in bearish_dir_cols) / F.col("DirectionalSumNonZero"))
...
# Finalize Signals
# -------------------------------
def finalize_signals(sdf, tf, tf_window=5, use_fundamentals=True, user: int = 1):
    """
    Spark-native consolidation of momentum + pattern + candle into unified Action.
    Works on a Spark DataFrame and avoids collect().
    """

    # --- Generate signal column names and momentum factor ---
    ...
    # --- Momentum scoring ---
    sdf = sdf.withColumn("Return", (F.col("Close") / F.lag("Close", 1).over(w) - 1).cast("double"))
    sdf = sdf.fillna({"Return": 0})
    sdf = sdf.withColumn("AvgReturn", F.avg("Return").over(w.rowsBetween(-9, 0)))
    sdf = sdf.withColumn("Volatility", F.stddev("Return").over(w.rowsBetween(-9, 0)))
    sdf = sdf.fillna({"Volatility": 1e-8})
    sdf = sdf.withColumn("MomentumZ", (F.col("Return") - F.col("AvgReturn")) / F.col("Volatility"))

    # --- Compute aggregated stats in Spark ---
    agg_df = sdf.agg(
        F.mean("MomentumZ").alias("mean_mom"),
        F.stddev("MomentumZ").alias("std_mom")
    )
    sdf = sdf.crossJoin(agg_df)
    buy_thresh = F.col("mean_mom") + momentum_factor * F.col("std_mom")
    sell_thresh = F.col("mean_mom") - momentum_factor * F.col("std_mom")
    ...
    bull_sum = pattern_sum(bullish_patterns)
    bear_sum = pattern_sum(bearish_patterns)
    sdf = sdf.withColumn("BullScore", bull_sum)
    sdf = sdf.withColumn("BearScore", bear_sum)
    ...
    sdf = sdf.withColumn(
        "CandidateAction",
        F.when(
            F.expr("array(MomentumAction, PatternAction, CandleAction)").contains("Buy"),
            "Buy"
        ).when(
            F.expr("array(MomentumAction, PatternAction, CandleAction)").contains("Sell"),
            "Sell"
        ).otherwise("Hold")
    )
    '''
    ...
    sdf = sdf.withColumn(
        ...
        F.when(
            F.array_contains(
                F.array(
                    F.col("MomentumAction").cast("string"),
                    F.col("PatternAction").cast("string"),
                    F.col("CandleAction").cast("string")
                ),
                "Buy"
            ),
            "Buy"
    ...
    sdf = sdf.withColumn("PrevAction", F.lag("CandidateAction", 1).over(w))
    sdf = sdf.withColumn(
        "Action",
        F.when(
            (F.col("CandidateAction") == F.col("PrevAction")) & F.col("CandidateAction").isin("Buy", "Sell"),
            "Hold"
    ...
    if valid_cols:
        ...
        # Compute maxes in Spark
        max_df = sdf.agg(
            F.max("BullishCount").alias("max_bull"),
            F.max("BearishCount").alias("max_bear"),
            F.max("MagnitudeStrength").alias("max_mag")
        )
        sdf = sdf.crossJoin(max_df)

        sdf = sdf.withColumn("BullishStrengthHybrid", (F.col("BullishCount") / F.col("max_bull")) + (F.col("MagnitudeStrength") / F.col("max_mag")))
        sdf = sdf.withColumn("BearishStrengthHybrid", (F.col("BearishCount") / F.col("max_bear")) + (F.col("MagnitudeStrength") / F.col("max_mag")))
        sdf = sdf.withColumn("SignalStrengthHybrid", F.greatest("BullishStrengthHybrid", "BearishStrengthHybrid"))

        # --- ActionConfidence with optional fundamentals ---
        sdf = sdf.withColumn("ActionConfidence", F.col("SignalStrengthHybrid"))
        
        max_conf = sdf.agg(F.max("ActionConfidence")).first()[0]
        sdf = sdf.withColumn("ActionConfidenceNorm", F.col("ActionConfidence") / F.lit(max_conf)) \
                 .withColumn("SignalDuration", F.sum(F.when(F.col("Action") !=    F.lag("Action").over(Window.orderBy("StockDate")),1).otherwise(0)).over(Window.orderBy("StockDate"))) \
                 .withColumn("ValidAction", F.col("Action").isin("Buy", "Sell")) \
                 .withColumn("HasValidSignal", F.col("Action").isNotNull() & F.col("TomorrowAction").isNotNull() & F.col("SignalStrengthHybrid").isNotNull())
